...
class WESPlatform(Platform):
    ...
    def connect(self, **kwargs):
        ...
        try:
            ...
            self.logger.info("Connected to WES API: %s", response.get('workflow_type_versions', {}))
            self.connected = True
            return True
    ...
    def download_file(self, file, dest_folder):
        ...
        if file.startswith('http'):
            ...
            response = requests.get(file, stream=True, timeout=60)
            response.raise_for_status()
    ...
    def get_task_output_filename(self, task, output_name):
        ...
        if isinstance(output, str) and (output.startswith('http') or output.startswith('file:')):
