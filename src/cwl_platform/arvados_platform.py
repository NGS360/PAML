...
class ArvadosPlatform(Platform):
    ...
    def _get_files_list_in_collection(
        ...
        """
        the_col = arvados.collection.CollectionReader(
            manifest_locator_or_text=collection_uuid
        )
    ...
    def _load_cwl_output(self, task: ArvadosTask):
        ...
        cwl_output_collection = arvados.collection.Collection(
            task.container_request["output_uuid"],
            api_client=self.api,
    ...
    def copy_folder(self, source_project, source_folder, destination_project):
        ...
        """
        self.logger.debug(
            "Copying folder %s from project %s to project %s",
            source_folder,
            source_project["uuid"],
        ...
        if len(search_result["items"]) > 0:
            self.logger.debug(
                "Found source collection %s in project %s",
                collection_name,
                source_project["uuid"],
            )
            ...
            self.logger.error(
                "Source collection %s not found in project %s",
                collection_name,
                source_project["uuid"],
            )
            return None
        ...
        if len(search_result["items"]) > 0:
            self.logger.debug(
                "Found destination folder %s in project %s",
                collection_name,
                destination_project["uuid"],
            )
            ...
            self.logger.debug(
                "Destination folder %s not found in project %s, creating",
                collection_name,
                destination_project["uuid"],
            )
        ...
        target_collection = arvados.collection.Collection(
            destination_collection["uuid"]
        )
    ...
    def copy_workflow(self, src_workflow, destination_project):
        ...
        """
        self.logger.debug(
            "Copying workflow %s to project %s",
            src_workflow,
            destination_project["uuid"],
        )
        ...
        if len(existing_workflows["items"]):
            self.logger.debug(
                "Workflow %s already exists in project %s",
                wf_name,
                destination_project["uuid"],
            )
        ...
        self.logger.debug(
            "Workflow %s does not exist in project %s, copying",
            wf_name,
            destination_project["uuid"],
        )
        ...
        self.logger.debug(
            "Copied workflow %s to project %s", wf_name, destination_project["uuid"]
        )
    ...
    def copy_workflows(self, reference_project, destination_project):
        ...
        reference_workflows = (
            self.api.workflows()
            .list(filters=[["owner_uuid", "=", reference_project["uuid"]]])
            .execute()
        )
        destination_workflows = (
            self.api.workflows()
            .list(filters=[["owner_uuid", "=", destination_project["uuid"]]])
            .execute()
        )
        ...
        for workflow in reference_workflows["items"]:
            if workflow["name"] not in [
                ...
                destination_workflows.append(
                    self.api.workflows().create(body=workflow).execute()
                )
    ...
    def create_project(self, project_name, project_description, **kwargs):
        ...
        project = (
            ...
            .create(
                body={
                    "owner_uuid": f'{arvados_user["uuid"]}',
                    "name": project_name,
                    "description": project_description,
                    "properties": {"proj_owner": arvados_user["username"]},
                    "group_class": "project",
                }
            )
            .execute()
    ...
    def delete_task(self, task: ArvadosTask):
        ...
        self.api.container_requests().delete(
    ...
    def get_current_task(self) -> ArvadosTask:
        ...
        request = (
            ...
            .list(filters=[["container_uuid", "=", current_container["uuid"]]])
            .execute()
        )
    ...
    def get_files(self, project, filter=None):
        ...
        if filter and "folder" in filter:
            collection_filter.append(["name", "=", filter["folder"]])
        self.logger.debug(
        ...
        for num, collection in enumerate(matching_collections):
            ...
            files += self._get_files_list_in_collection(
                collection["uuid"], filters=filter
            )
    ...
    def get_tasks_by_name(self, project, task_name):  # -> list(ArvadosTask):
        ...
        for container_request in arvados.util.keyset_list_all(
            ...
            container = (
                self.api.containers()
                .get(uuid=container_request["container_uuid"])
                .execute()
            )
    ...
    def get_project_by_name(self, project_name):
        ...
        search_result = (
            self.api.groups().list(filters=[["name", "=", project_name]]).execute()
        )
        if len(search_result["items"]) > 0:
    ...
    def get_project_by_id(self, project_id):
        ...
        search_result = (
            self.api.groups().list(filters=[["uuid", "=", project_id]]).execute()
        )
        if len(search_result["items"]) > 0:
            return search_result["items"][0]
        return None
    ...
    def get_user(self, user):
        ...
        if "@" in user:
            user_resp = (
                self.api.users().list(filters=[["email", "ilike", user]]).execute()
            )
        else:
            user_resp = (
                self.api.users().list(filters=[["username", "ilike", user]]).execute()
            )
    ...
    def stage_output_files(self, project, output_files):
        ...
        if len(search_result["items"]) > 0:
            ...
            outputs_collection = (
                self.api.collections()
                .create(
                    body={"owner_uuid": project["uuid"], "name": output_collection_name}
                )
                .execute()
            )
        outputs_collection = arvados.collection.Collection(
            outputs_collection["uuid"], api_client=self.api
        )
        ...
        with outputs_collection.open("sources.txt", "a+") as sources:
            ...
            for output_file in output_files:
                ...
                source_collection_uuid = (
                    output_file["source"].split(":")[1].split("/")[0]
                )
                source_file = "/".join(
                    output_file["source"].split(":")[1].split("/")[1:]
                )
                source_collection = arvados.collection.Collection(
                    source_collection_uuid, api_client=self.api
                )

                # Copy the file
                outputs_collection.copy(
                    source_file,
                    target_path=output_file["destination"],
                    source_collection=source_collection,
                    overwrite=True,
                )
    ...
    def stage_task_output(self, task, project, output_to_export, output_directory_name):
        ...
        if len(search_result["items"]) > 0:
            ...
            outputs_collection = (
                self.api.collections()
                .create(
                    body={"owner_uuid": project["uuid"], "name": output_directory_name}
                )
                .execute()
            )
    ...
    def submit_task(self, name, project, workflow, parameters, execution_settings=None):
        ...
        with tempfile.NamedTemporaryFile() as parameter_file:
            ...
            use_spot_instance = (
                execution_settings.get("use_spot_instance", True)
    ...
    def upload_file_to_project(
        ...
        if len(search_result["items"]) > 0:
            ...
            destination_collection = (
                self.api.collections()
                .create(body={"owner_uuid": project["uuid"], "name": collection_name})
                .execute()
            )
